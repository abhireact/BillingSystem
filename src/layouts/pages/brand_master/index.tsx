...
const View = () => {
  ...
  const handleDeleteData = async (row: any) => {
    ...
    try {
      const deletedata = await axios.delete("http://10.0.20.121:8000/brands", {
        data: {
          ...row,
        },
        headers: {
  ...
  useEffect(() => {
    axios
      .get("http://10.0.20.121:8000/brands", {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
  ...
  const dataTableData = {
    ...
    rows: data.map((row, index) => ({
      brand_name: <MDTypography variant="p">{row.brand_name}</MDTypography>,
  ...
  return (
    <DashboardLayout>
      ...
      <Grid sx={{ display: "flex", justifyContent: "space-between" }}>
        <MDTypography>Brand Master</MDTypography>
